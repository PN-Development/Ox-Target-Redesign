...
local function AttemptDoorToggle(vehicle, door)
    if NetworkGetEntityOwner(vehicle) ~= cache.playerId then
        TriggerServerEvent('nxt-misc:server:requestDoorToggle', VehToNet(vehicle), door)
        return
    end

    toggleDoor(vehicle, door)
end

RegisterNetEvent('nxt-misc:client:toggleDoor', function(vehicleNetId, doorIndex)
    local vehicle = NetworkGetEntityFromNetworkId(vehicleNetId)
    toggleDoor(vehicle, doorIndex)
end)

AddEventHandler('nxt-misc:client:attemptDoorToggle', function(vehicle, doorIndex)
    toggleDoor(vehicle, doorIndex)
end)
...
api.addGlobalVehicle({
    {
        ...
        onSelect = function(data)
            AttemptDoorToggle(data.entity, 0)
        end
    },
    {
        ...
        onSelect = function(data)
            AttemptDoorToggle(data.entity, 1)
        end
    },
    {
        ...
        onSelect = function(data)
            AttemptDoorToggle(data.entity, 2)
        end
    },
    {
        ...
        onSelect = function(data)
            AttemptDoorToggle(data.entity, 3)
        end
    },
    {
        ...
        onSelect = function(data)
            AttemptDoorToggle(data.entity, 4)
        end
    },
    {
        ...
        onSelect = function(data)
            AttemptDoorToggle(data.entity, 5)
        end
    }
})
